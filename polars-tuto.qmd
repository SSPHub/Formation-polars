...
L'option `schema_overrides` est ici nécessaire sous peine d'une erreur qu'on ne rencontrait pas
...
Pour sélectionner les observations où la variable TYPEQU a une valeur manquante:

```{python}
# Valeurs manquantes pour la variable EPCI
df_bpe.filter(
    pl.col("TYPEQU").is_null()
)
```

A l'inverse, si on désire ne sélectionner que les
observations où la colonne _"TYPEQU"_ n'est pas manquante :

```{python}
df_bpe.filter(
    pl.col("TYPEQU").is_not_null()
)
```
